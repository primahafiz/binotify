...
let order_by_year = find_get_parameter('order_by_year')
let is_ascending = find_get_parameter('is_ascending')
...
function ajax_req(){
    ...
    if (query != null && query!='') sql_query = sql_query.concat(`&query=${query}`);
    if (is_ascending != null && is_ascending!='') sql_query = sql_query.concat(`&is_ascending=${is_ascending}`);
    if (order_by_year != null && order_by_year!='') sql_query = sql_query.concat(`&order_by_year=${order_by_year}`);
...
const form = document.querySelector("#formFilterSort")
...
form.addEventListener('submit',function(e){
    ...
    window.location = `${BASE_URL}/search?is_ascending=${formData.get('titleMode')}&order_by_year=${formData.get('yearMode')}&query=${formData.get('search')}`
})